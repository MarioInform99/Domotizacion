...
if(array_key_exists('Authorization',$headerAll) && !empty(trim($headerAll['Authorization']))){
        ...
        if(isset($token[1]) && !empty(trim($token[1]))){
            ...
            $query=$conexion->prepare('SELECT `usuarios`.`NOMBRE`,`usuarios`.`CORREO`,`usuarios`.`APELLIDOS`, `mensaje-user`.* FROM `mensaje-user`, `usuarios` WHERE  `mensaje-user`.`ID-USER`=`usuarios`.`ID` ;');
            $query->execute();
